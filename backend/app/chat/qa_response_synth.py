...
A user has selected a set of Indian financial documents (annual reports, NSE filings, \
investor presentations) and has asked a question about them.
...
When reporting financial figures, preserve units exactly (₹ Crore, ₹ Lakh, USD Million, etc.) \
and always mention the relevant financial year.
Refined Answer:
""".strip()
...
When reporting financial figures, preserve units exactly (₹ Crore, ₹ Lakh, USD Million, etc.) \
and always mention the relevant financial year.
Do not fabricate numbers. If the answer is not in the context, say so.
Query: {{query_str}}
Answer:
""".strip()
